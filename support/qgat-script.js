...
// ========================      QGAT DEMO        =======================
// =============================================================
function funcDemoSubmit(){
	
	var tmpURL = document.getElementById('txtURL').value;
	var tmpLimitAsin = document.getElementById('txtLimitAsin').value;
	var tmpResult = document.getElementById('lstResult').value;
	var tmpCatName = document.getElementById('txtFileName').value;
	
	if(tmpURL  == ""){
		alert("Require QGAT URL");
		document.getElementById('txtURL').focus();
		return;
	}
	
	if(tmpLimitAsin  == ""){
		alert("Require QGAT LIMIT ASIN");
		document.getElementById('txtLimitAsin').focus();
		return;
	}	

	if(tmpCatName  == ""){
		alert("Require QGAT CATEGORY NAME");
		document.getElementById('txtFileName').focus();
		return;
	}		

	window.open('qgat-demo-gen.php?tmpURL=' + encodeURIComponent(tmpURL) + "&tmpLimitAsin=" + tmpLimitAsin + "&tmpResult=" + tmpResult + "&tmpCatName=" + encodeURIComponent(tmpCatName));
...
// ################ STARTGET DEMO #####################
function funcDemoGet(tmpPage){	


	var tmpURL = document.getElementById('txtURL').value;
	var tmpSelectURL = document.getElementById('txtSelectURL').value;	
	var tmpLimitAsin = document.getElementById('txtLimitAsin').value;	
	...
	if(tmpSelectURL == ""){		
		...
		var tmpSelectURL = tmpArrayURL[0];	
		document.getElementById('txtSelectURL').value	= tmpSelectURL;
	}
		
	var tmpLimitAsin = document.getElementById('txtLimitAsin').value;
	var tmpResultPage = document.getElementById('txtResultPage').value;
	//var tmpCatName = document.getElementById('txtFileName').value;	
	...
	if(tmpCatName  == ""){
		alert("Require Export File Name");
		document.getElementById('txtFileName').focus();
		return;
	}	*/	
	...
	var tmpData = tmpSelectURL + "|||" + tmpLimitAsin + "|||" + tmpResultPage + "|||" + tmpPage + "|||" + tmpMainCount + "|||" + tmpMaxPage;	
	var totalData = encodeURIComponent(tmpData);	
	document.getElementById('divLoad').style.display = 'block';	
	
	createXMLHttpRequest();
	xmlHttp.onreadystatechange = stateFuncDemoGet;	
	xmlHttp.open("POST","qgat-demo-get.php?dummy=" + Math.random(),true);
	xmlHttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"); 
	xmlHttp.send("totalData=" + totalData); 	
	
}

function stateFuncDemoGet(){
	if(xmlHttp.readyState==4 && xmlHttp.status==200){				
		var tmpMainData = xmlHttp.responseText;		
		var tmpDetail = tmpMainData.split("|||");
		
		var tmpCountNoSame = document.getElementById('txtLimitAsin').value;	
		...
		if(tmpPage == 2){
			//document.getElementById('txtFileName').value =tmpCategoryName ;	
		...
		document.getElementById('txtLimitAsin').value = tmpCountNoSame;
		document.getElementById('divTotalCount').innerHTML =tmpCountNoSame ; 	
		...
		if(tmpFinish == "1"){ 							
			...
			funcDemoSaveCSV();
			//funcDemoSaveCSV();  //1 sub category , 2 all category
